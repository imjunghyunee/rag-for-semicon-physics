...
Respond with ONLY the numbered list of sub-questions."""
                ...
                {
                    "role": "user", 
                    "content": f"Break down this complex semiconductor physics question: {original_query}"
                }
            ],
            max_tokens=1000,
            temperature=0.3
        ...
        if not subquestions:
            print("Warning: No valid sub-questions generated, creating fallback questions")
...
def process_subquestion(
    ...
    if retrieval_type == "hyde" and hybrid_weights:
        context_docs, explanation = retrievers.hyde_hybrid_retrieve(subquestion, weights=hybrid_weights)
        ...
        context_docs, explanation = retrievers.summary_hybrid_retrieve(subquestion, weights=hybrid_weights)
    elif retrieval_type == "summary":
        context_docs, explanation = retrievers.summary_retrieve(subquestion)
    elif hybrid_weights:
        context_docs = retrievers.vectordb_hybrid_retrieve(subquestion, weights=hybrid_weights)
        explanation = ""
    else:
    ...
    context_contents = [doc.page_content for doc in context_docs if isinstance(doc, Document)]
...
def aggregate_subquestion_results(
    ...
    for i, result in enumerate(subquestion_results, 1):
        ...
        combined_answers += f"{i}. Q: {result['question']}\n   A: {result['answer']}\n\n"
    
    try:
        final_answer = utils.client.chat.completions.create(
            model=config.OPENAI_MODEL,
            messages=[
                {
                    "role": "system",
...
def process_complex_query(
    ...
    try:
        ...
        if not subquestions:
            print("Warning: No sub-questions generated, falling back to simple processing")
            # 폴백: 원래 질문을 그대로 처리
            return {
        ...
        for i, subquestion in enumerate(subquestions, 1):
            try:
                ...
                result = process_subquestion(subquestion, retrieval_type, hybrid_weights)
        ...
        try:
            final_answer = aggregate_subquestion_results(original_query, subquestion_results)
        except Exception as e:
            print(f"Error in aggregation: {e}")
            # 폴백: 간단한 답변 조합
            answers = [result["answer"] for result in subquestion_results if result["answer"]]
